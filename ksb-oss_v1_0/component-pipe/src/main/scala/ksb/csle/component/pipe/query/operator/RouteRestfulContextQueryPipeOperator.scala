...
import ksb.csle.component.pipe.controller.vo.RouteContext
 ...
 */
class RouteRestfulContextQueryPipeOperator[T](
  ...
  override def query(ctx: BaseRouteContext[T]): BaseRouteContext[T] = {
    val outCtx = ctx.getRoute() match {
      case Some(url) =>
        super.queryUrl(url, ctx)
      case _ =>
        super.query(ctx)
    }
    outCtx.setRoute(outCtx.getOutput())
    outCtx
  }
}
