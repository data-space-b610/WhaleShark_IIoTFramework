 ...
 *
 * After sending a post request to the http server,
...
final class HttpDeleteResponseReceiver(
    val name: String,
    val url: String,
    val params: Map[String, String],
    val headers: Map[String, String]) extends Receiver[String](StorageLevel.MEMORY_AND_DISK_2) with Logging {
  ...
  private def receive(): StopArgs = {
    ...
    val result = deleteOperation(request)
  ...
  private def deleteOperation(wsRequest: WSRequest): WSResponse = {
