...
#--output /tmp/csle-tf/predict/rnn/predict_rnn.csv
...
def train_or_predict():
    ...
    # create keep_prob as constant for serving.
...
#     rnn_last_outputs = tf.squeeze(rnn_last_outputs)
    ...
    if FLAGS.isTrain:
        ...
        p = os.path.join(FLAGS.model, FLAGS.model_path)
...
#                plot_status(sess, datasets, x, y, total_loss, unscaled_output, loss_weights, keep_prob)
        ...
        tfutil.export_model(FLAGS.export_url,
                            FLAGS.hdfs_user,
                            FLAGS.model_version,
...
#     parser.add_argument('--c', help='# of cols')
    ...
    # for serving
    parser.add_argument("--export_url", type=str, default="/tmp/rnn2",
                        help="url to export a saved model")
    parser.add_argument("--hdfs_user", type=str, default="csle",
                        help="if export to hdfs, set hdfs user account")
    parser.add_argument("--model_version", type=str, default="001",
                        help="model version")
    parser.add_argument("--signature_name", type=str, default="predict_speed",
                        help="name of signature to serve")
