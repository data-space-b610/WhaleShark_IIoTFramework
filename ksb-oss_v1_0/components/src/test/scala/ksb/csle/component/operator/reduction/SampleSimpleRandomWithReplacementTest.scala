...
/**
 * Test Class for SampleSimpleRandomWithReplacementTest function in data sampling package.
 */
case class Data(data1: Double, data2: Double, data3: Int, data4: Double, data5: Double)

class SampleSimpleRandomWithReplacementTest extends UnitSpec with DataFunSuite with Logging {
  ...
  "A SampleSimpleRandomWithReplacement" should "do SRSWR with EQUAL condition" in {
    val sampleSimpleRandomWithReplacementInfo = SampleSimpleRandomInfo.newBuilder()
      .setFraction(0.5)
    ...
    val operator1 = StreamOperatorInfo.newBuilder()
      ...
      .setSampleSimpleRandom(sampleSimpleRandomWithReplacementInfo)
      .build
    val w = Window.partitionBy(lit(1)).orderBy(lit(1))
    val srcDF = convertDF(inputData)
    var correctAnswerDf = convertDF(correctAnswer)
      .withColumn("index", row_number.over(w))
    val sampleSimpleRandomWithReplacement = RandomSamplingOperator(operator1)
    var desDF = sampleSimpleRandomWithReplacement
      .operate(srcDF)
      .withColumn("index", row_number.over(w))
    ...
    resultDF.select(
      ...
      desDF.col("data5"), correctAnswerDf.col("data5")).collect().foreach {
        case Row(a: Double, b: Double, c: Double, d: Double, e: Integer,
          ...
          "The feature value is not correct after SampleSimpleRandomWithReplacement.")
    }
    logger.info(resultDF.show.toString)
  }
}
