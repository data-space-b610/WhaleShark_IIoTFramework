 ...
 */
object BatchAutoMLTrainInSingleEngine extends Logging {
  ...
  private def workflow: WorkflowInfo = {
    val infileInfo = FileInfo.newBuilder()
      .addFilePath(
          "dataset/BatchAutoMLTrainInSingleEngine/hue_train_dataset"
          .replaceAll("\\\\", "/"))
      .setFileType(FileInfo.FileType.PARQUET)
      .build
      val outfileInfo = FileInfo.newBuilder()
      .addFilePath("examples")
      ...
      .setPySparkMLRunner(
          ...
          .setPyEntryPath(
              "file:////home/csle/ksb-csle/pyML/autosparkml/bridge/call_trainer.py")
          .setInJson(false)
          .setSparkArgs(
...
//            .setExecutorCores("2"))
      ...
      .addEngines(
          ...
          .setId(1)
          .setPrevId(0)
          .setEngineNickName("AutoMLEngine")
