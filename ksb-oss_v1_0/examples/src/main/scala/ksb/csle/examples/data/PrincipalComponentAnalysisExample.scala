...
/**
 * Object to create PrincipalComponentAnalysis workflow scenario as a protobuf message WorkflowInfo.
 * See {@link WorkflowInfo}.
 * This is used for test of PrincipalComponentAnalysis workflow scenario.
 *
 * TODO: Support graphical user interface for easy workflow editing.
 */
object PrincipalComponentAnalysisExample extends Logging {
  ...
  private def workflow = {
     ...
     val infileInfo = FileInfo.newBuilder()
      .addFilePath(
        s"file:///${System.getProperty(
            "user.dir")}/input/input_principalcomponentanalysis.csv"
        .replaceAll("\\\\", "/"))
    ...
    val outfileInfo = FileInfo.newBuilder()
      .addFilePath(
        s"file:///${System.getProperty("user.dir")}/output/result_principalcomponentanalysis.csv"
        .replaceAll("\\\\", "/"))
      .setFileType(FileInfo.FileType.CSV)
      .setDelimiter(",")
      .build
    val operator1 = StreamOperatorInfo.newBuilder()
      .setId(2)
      .setPrevId(1)
      .setClsName("ksb.csle.component.operator.reduction.PCAOperator")
