 ...
 */
object FilterExample extends Logging {
  val appId: String = "Data-FilterExample"
...
//        .setVal(8)
//        .setColName("word")
//        .setCondition(FilterInfo.Condition.EXIST)
//        .setPattern("ppy")
        .setColName("word")
        .setCondition(FilterInfo.Condition.LIKE)
        .setPattern("ppy")
        .build()
    val outfileInfo = FileInfo.newBuilder()
        .addFilePath(s"file:///${System.getProperty("user.dir")}/output/result_filter.csv".replaceAll("\\\\", "/"))
        .setFileType(FileInfo.FileType.CSV)
        .setDelimiter(",")
        .addField(
          FieldInfo.newBuilder()
