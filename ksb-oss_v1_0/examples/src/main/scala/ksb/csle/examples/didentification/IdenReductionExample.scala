...
object IdenReductionExample extends BasePrivacyWorkflow {
  val appId: String = "IdenReduction"
  ...
  private def workflow = {
    ...
    val operator = getDidentOperator()
      .setClsName("ksb.csle.didentification.privacy.IdenReductionOperator")
      .setIdenReduction(reductionInfo)
      .build

    val controller = getStreamController
    val runner = getSimpleSparkRunner(appId)
    val writer = getFileWriter(s"output/IdenReduction.csv")
