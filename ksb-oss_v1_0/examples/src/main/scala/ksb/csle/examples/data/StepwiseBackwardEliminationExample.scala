...
/**
 * Object to create StepwiseBackwardElimination workflow scenario as a protobuf message WorkflowInfo.
 * See {@link WorkflowInfo}.
 * This is used for test of StepwiseBackwardElimination workflow scenario.
 *
 * TODO: Support graphical user interface for easy workflow editing.
 */
object StepwiseBackwardEliminationExample extends Logging {
  val appId: String = "Data-StepwiseBackwardEliminationExample"
  ...
  private def workflow = {
    ...
    val infileInfo = FileInfo.newBuilder()
      .addFilePath(
        s"file:///${System.getProperty(
            "user.dir")}/input/input_stepwisebackwardelimination.csv"
        .replaceAll("\\\\", "/"))
    ...
    val outfileInfo = FileInfo.newBuilder()
      .addFilePath(
        s"file:///${System.getProperty("user.dir")}/output/result_stepwisebackwardelimination.csv"
        .replaceAll("\\\\", "/"))
      .setFileType(FileInfo.FileType.CSV)
      .setDelimiter(",")
      .build
    val operator1 = StreamOperatorInfo.newBuilder()
      .setId(2)
      .setPrevId(1)
      .setClsName("ksb.csle.component.operator.reduction.StepwiseBackwardEliminateOperator")
