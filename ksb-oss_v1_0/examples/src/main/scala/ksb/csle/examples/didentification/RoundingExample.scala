...
object RoundingExample extends BasePrivacyWorkflow {
  val appId: String = "Rounding"
...
//      .setSelectedColumnId(randomNumber)
      .setRoundStep(20)
      .setMethod(RoundingMethod.ROUND_DOWN)
      .build()
      
    val operator = getDidentOperator()
      .setClsName("ksb.csle.didentification.privacy.RoundingOperator")
      .setRounding(roundingInfo)
      .build

    val controller = getStreamController
    val runner = getSimpleSparkRunner(appId)
    val writer = getFileWriter(s"output/Rounding.csv")
