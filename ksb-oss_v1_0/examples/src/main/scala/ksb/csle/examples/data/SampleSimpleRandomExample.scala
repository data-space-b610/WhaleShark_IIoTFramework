...
/**
 * Object to create SampleSimpleRandom workflow scenario as a protobuf message WorkflowInfo.
 * See {@link WorkflowInfo}.
 * This is used for test of SampleSimpleRandom workflow scenario.
 *
 * TODO: Support graphical user interface for easy workflow editing.
 */
object SampleSimpleRandomExample extends Logging {
  val appId: String = "Data-SampleSimpleRandomExample"
  ...
  private def workflow = {
    ...
    val infileInfo = FileInfo.newBuilder()
      .addFilePath(
        s"file:///${System.getProperty("user.dir")}/input/input_simplerandomsampling.csv"
        .replaceAll("\\\\", "/"))
    ...
    val outfileInfo = FileInfo.newBuilder()
      .addFilePath(
        s"file:///${System.getProperty("user.dir")}/output/result_simplerandomsampling.csv"
        .replaceAll("\\\\", "/"))
      .setFileType(FileInfo.FileType.CSV)
      .setDelimiter(",")
      .build
    val operator1 = StreamOperatorInfo.newBuilder()
      .setId(2)
      .setPrevId(1)
      .setClsName("ksb.csle.component.operator.reduction.RandomSamplingOperator")
      .setSampleSimpleRandom(sampleSimpleRandomInfo)
      .build
