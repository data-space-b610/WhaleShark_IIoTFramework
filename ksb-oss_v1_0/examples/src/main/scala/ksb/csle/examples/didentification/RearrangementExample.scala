...
object RearrangementExample extends BasePrivacyWorkflow {
  val appId: String = "Rearrangement"
  ...
  private def workflow = {
    ...
    val rearrangeColumns = List(0, 1, 2)
    rearrangeColumns.map(x => rearrangeInfo.addSelectedColumnId(x))
      
    val operator = getDidentOperator()
      .setClsName("ksb.csle.didentification.privacy.RearrangeOperator")
      .setRearrange(rearrangeInfo)
      .build

    val controller = getStreamController
    val runner = getSimpleSparkRunner(appId)
    val writer = getFileWriter(s"output/Rearrangement.csv")
