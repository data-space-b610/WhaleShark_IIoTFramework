...
object RandomNoiseExample extends BasePrivacyWorkflow {
  val appId: String = "RandomNoise"
  ...
  private def workflow = {
    ...
    val numHandleInfo = NumericHandle.newBuilder()
      .setIsRandom(RandomType.GAUSSIAN)
      .setOperator(NoiseOperator.NOISE_MINUS)
    ...
    val operator = getDidentOperator()
      .setClsName("ksb.csle.didentification.privacy.RandomNoiseOperator")
      .setRandomNoise(noiseInfo)
      .build

    val controller = getStreamController
    val runner = getSimpleSparkRunner(appId)
    val writer = getFileWriter(s"output/RandomNoise.csv")
