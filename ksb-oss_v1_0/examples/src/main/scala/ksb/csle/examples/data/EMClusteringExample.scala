...
/**
 * Object to create EMClustering workflow scenario as a protobuf message WorkflowInfo.
 * See {@link WorkflowInfo}.
 * This is used for test of EMClustering workflow scenario.
 *
 * TODO: Support graphical user interface for easy workflow editing.
 */
object EMClusteringExample extends Logging {
  val appId: String = "Data-EMClusteringExample"
  ...
  private def workflow = {
    val eMClusteringInfo = EMClusteringInfo.newBuilder()
        .setKValue(2)
        .setMaxIter(100)
        .build
    val infileInfo = FileInfo.newBuilder()
      .addFilePath(
        s"file:///${System.getProperty(
            "user.dir")}/input/input_emclustering.csv"
        .replaceAll("\\\\", "/"))
    ...
    val outfileInfo = FileInfo.newBuilder()
      .addFilePath(
        s"file:///${System.getProperty("user.dir")}/output/result_emclustering.csv"
        .replaceAll("\\\\", "/"))
      .setFileType(FileInfo.FileType.CSV)
      .setDelimiter(",")
      .build
    val operator1 = StreamOperatorInfo.newBuilder()
      .setId(2)
      .setPrevId(1)
      .setClsName("ksb.csle.component.operator.cleaning.EMClusteringOperator")
