...
/**
 * Object to create LinearRegression workflow scenario as a protobuf message WorkflowInfo.
...
object LinearRegressionExample extends Logging {
  ...
  private def workflow = {
    ...
    val infileInfo = FileInfo.newBuilder()
      .addFilePath(
        s"file:///${System.getProperty(
            "user.dir")}/input/input_linearregression.csv"
        .replaceAll("\\\\", "/"))
    ...
    val outfileInfo = FileInfo.newBuilder()
      .addFilePath(
        s"file:///${System.getProperty("user.dir")}/output/result_linearregression.csv"
        .replaceAll("\\\\", "/"))
      .setFileType(FileInfo.FileType.CSV)
      .setDelimiter(",")
      .build
    val operator1 = StreamOperatorInfo.newBuilder()
      .setId(2)
      .setPrevId(1)
      .setClsName("ksb.csle.component.operator.cleaning.LinearRegressionOperator")
