...
object DataRangeExample extends BasePrivacyWorkflow {
  val appId: String = "DataRange"
  ...
  private def workflow = {
    ...
    val dataRangeInfo = DataRangeInfo.newBuilder()
    ...
    val operator = getDidentOperator()
      .setClsName("ksb.csle.didentification.privacy.DataRangeOperator")
      .setDataRange(dataRangeInfo)
      .build

    val controller = getStreamController
    val runner = getSimpleSparkRunner(appId)
    val writer = getFileWriter(s"output/DataRange.csv")
