...
object RandomRoundingExample extends BasePrivacyWorkflow {
  val appId: String = "RandomRounding"
  ...
  private def workflow = {
    ...
    val roundingInfo = RandomRoundingInfo.newBuilder()
      ...
      .setRoundStep(5)
      .setMethod(RoundingMethod.ROUND_UP)
      .build()
      
    val operator = getDidentOperator()
      .setClsName("ksb.csle.didentification.privacy.RandomRoundingOperator")
      .setRandomRounding(roundingInfo)
      .build

    val controller = getStreamController
    val runner = getSimpleSparkRunner(appId)
    val writer = getFileWriter(s"output/RandomRounding.csv")
